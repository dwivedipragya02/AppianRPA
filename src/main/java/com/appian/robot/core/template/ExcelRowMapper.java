...
public class ExcelRowMapper implements IRowMapper<IExcel, ExcelRow> {
	...
	public ExcelRow map(IExcel data, int rowNum) {
		...
		r.setCol4(data.getCellValueAsString(rowNum, COL_4));
	...
	public void update(IExcel data, int rowNum, ExcelRow rowData) {
		...
		data.setCellValueByRef(new CellReference(rowNum, COL_4), rowData.getCol4());
	...
	/**
	 * The last row is determined when the first row without content in the
	 * first column is detected.
	 * <p>
	 * Another possibility could be to check also the second and the third
	 * columns.
	 * 
