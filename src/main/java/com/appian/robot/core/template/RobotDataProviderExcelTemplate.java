...
public class RobotDataProviderExcelTemplate implements IRobot {
	 ...
	 * Action "startUp".
	 * <p>
	 * This method is overwritten to initialize the Appian RPA modules
	 * instances.
	 */
	@Override
	...
	public void start() throws Exception {
		...
		// Initialization of the Data Provider module using the RowMapper
		// implemented
	 ...
	 * Processes an item.
	 * 
	 * In this template example, the processing consists of concatenating the
	 * first 3 columns to get the string result and update the last column.
	 */
	public void processItem() {
		...
		// The process is very simple: to concatenate the 3 columns to get the
		// result
		...
		data.add(currentItem.getCol1());
		data.add(currentItem.getCol2());
		data.add(currentItem.getCol3());
		data.add(currentItem.getCol4());
		arr.add(data);
	}
	...
	public void openBrowser() {
		...
		driver.get("https://yexledemo.appiancloud.com/suite/sites/ycms/page/home");
		driver.manage().window().maximize();
		driver.manage().timeouts().implicitlyWait(2000, TimeUnit.SECONDS);

		DataProcess proce = new DataProcess();
		for (int i = 0; i < arr.size(); i++) {
			proce.performAction(driver, arr.get(i).get(0), arr.get(i).get(1), arr.get(i).get(2), arr.get(i).get(3));
		}
	}
	 ...
	 * 
	 * It is a private method to be called during the clean up to assure the
	 * data provider is correctly closed.
	 * 
	 * @throws IOException
	 ...
	 * Cleans up.
	 * 
	 * Besides returning the updated Excel file, it closes the data provider.
	 *
	...
	public String manageException(String action, Exception exception) throws Exception {
		...
		/*
		 * We take advantage of the Apache Commons ExceptionUtils class to know
		 * if a specific exception was thrown throughout the code. Since we
		 * threw a JidokaItemException, it's the one to be searched in the
		 * exceptions stack trace. If found, the flow goes to the next item by
		 * telling the next method to execute is 'moreItems()'. If another
		 * exception is found, it is propagated, so the robot ends with a
		 * failure.
		 */
